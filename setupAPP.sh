...
mode_advanced(){
  while true; do
    ...
    if $WANT_HY2; then
      printf "  ${C2}10)${C0} HY2 Cert Type : ${CC}%s${C0}\n" "${HY2_CERT:-[Not Set]}" >"$TTY"
      printf "  ${C2}11)${C0} HY2 Domain    : ${CC}%s${C0}\n" "${HY2_DOMAIN:-[Not Set]}" >"$TTY"
      printf "  ${C2}12)${C0} Let's Encrypt Email: ${CC}%s${C0}\n" "${LE_EMAIL:-[Not Set]}" >"$TTY"
    fi
    ...
    case "$opt" in
      ...
      11)
        if $WANT_HY2; then
          ask_valid HY2_DOMAIN "Enter Hysteria2 Domain" "$RE_DOMAIN" "${HY2_DOMAIN:-$DOMAIN}"
        else
          warn "Hysteria2 is disabled."
          press_enter
        fi
      ...
      12)
        if $WANT_HY2; then
          ask_valid LE_EMAIL "Enter Let's Encrypt Email" "$RE_EMAIL" "${LE_EMAIL:-}"
        else
          warn "Hysteria2 is disabled."
          press_enter
        fi
...
EOF
  ...
  local sub_locations=""
  IFS=',' read -ra TOKENS <<< "$SUB_TOKEN"
  for token in "${TOKENS[@]}"; do
    [ -z "$token" ] && continue
    sub_locations+=$(cat <<EOF
    location = /sub/${token} {
        default_type text/plain;
        alias /var/www/sub/${token}.txt;
    }
EOF
)
    sub_locations+=$'\n'
...
gen_subscription(){
  ...
  for token in "${TOKENS[@]}"; do
    [ -z "$token" ] && continue
    printf '%s\n' "${LINKS[@]}" | base64 -w0 > "/var/www/sub/${token}.txt" 2>/dev/null || printf '%s\n' "${LINKS[@]}" | base64 | tr -d '\n' > "/var/www/sub/${token}.txt"
    SUB_URLS+=("https://${DOMAIN}:${NGINX_PORTS[0]}/sub/${token}")
  done
  ok "Subscription file(s) created."
}
...
print_summary(){
  ...
  if $USE_DOMAIN; then
    ...
    printf "${CG}Subscription URLs:${C0}\n" >"$TTY"
...
import_config(){
  ...
  printf "\n${C3}Please confirm details for the new server:${C0}\n" >"$TTY"
  
  # 1. Main Domain Link
  local old_domain="${DOMAIN:-}"
  ask_valid DOMAIN "New Main Domain (for WS/XHTTP/Reality)" "$RE_DOMAIN" "${DOMAIN:-}"
  # Update PORT_IPS if it was pointing to old domain
  local p_idx
  for p_idx in "${NGINX_PORTS[@]:-}"; do
    if [ "${PORT_IPS[$p_idx]:-}" = "$old_domain" ]; then
      PORT_IPS[$p_idx]="$DOMAIN"
    fi
  done

  # 2. Hysteria2 Domain & Email
  if $WANT_HY2; then
    ask_valid HY2_DOMAIN "New HY2 Domain" "$RE_DOMAIN" "${HY2_DOMAIN:-$DOMAIN}"
    if [ "${HY2_CERT:-self}" = "le" ]; then
      ask_valid LE_EMAIL "New Let's Encrypt Email" "$RE_EMAIL" "${LE_EMAIL:-}"
    fi
  fi

  # 3. New Config Name
  ask CONFIG_NAME "New Config Name" "${CONFIG_NAME:-MyVPN}"
  CONFIG_NAME="${CONFIG_NAME// /_}"

  # 4. New Sub Path
  ask_valid SUB_PATH_IN "New Subscription Path" "$RE_SUBPATH" "${SUB_PATH_IN:-sub}"
  SUB_TOKEN="$SUB_PATH_IN"
...
main(){
  if [ "${1:-}" = "--headless" ]; then
    ...
    if ! load_state; then
      die "No state.env found in /etc/vpn-installer/ to rebuild."
    fi
    execute_build
    exit 0
  fi
